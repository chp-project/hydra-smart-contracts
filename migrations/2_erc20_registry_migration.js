...
module.exports = function(deployer, network) {
  ...
  deployer.deploy(TierionNetworkToken).then(function() {
    fs.writeFileSync('../e2e-testing/artifacts/ropsten_token.txt', TierionNetworkToken.address, 'utf8');
    return deployer.deploy(ChainpointRegistry, TierionNetworkToken.address);
  }).then(function() {
    fs.writeFileSync('../e2e-testing/artifacts/ropsten_registry.txt', ChainpointRegistry.address, 'utf8');
    return deployer.deploy(ChainpointQuorum, TierionNetworkToken.address, ChainpointRegistry.address);
  }).then(() => {
    fs.writeFileSync('../e2e-testing/artifacts/ropsten_quorum.txt', ChainpointQuorum.address, 'utf8');
    return;
  })
};